...
public class PassiveModeMeta implements MetadataValue {

	public static final String METADATA_NAME = "PASSIVE_MODE";
	
	protected Plugin plugin;
	protected boolean enabled;
	
	public PassiveModeMeta(Plugin plugin) {
		this.plugin = plugin;
		enabled = false;
	}
	...
	/**
	 * Находится ли игрок в пассивном режиме
	 * @param player
	 * @return
	 */
	public static boolean isModeEnabled(Player player) {
	 ...
	 * @return
	 */
	public static PassiveModeMeta from(Player player, Plugin plugin) {
		List<MetadataValue> metadata = player.getMetadata(METADATA_NAME);
		for (MetadataValue value : metadata) {
			if (value instanceof PassiveModeMeta)
				return (PassiveModeMeta)value;
		}
		PassiveModeMeta meta = new PassiveModeMeta(plugin);
